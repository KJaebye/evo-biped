...
class EvoBipedalWalkerPolicy(nn.Module):
    def __init__(self, cfg, agent):
        ...
        self.control_action_log_std = nn.Parameter(
            torch.ones(1, self.sim_action_dim) * self.cfg.policy_spec['control_log_std'])
    ...
    def forward(self, x):
        stages = ['scale_transform', 'execution']
        ...
        if len(x_dict['scale_transform']) > 0:
            stage_ind, scale_state, sim_obs = self.batch_data(x_dict['scale_transform'])
            x = scale_state
            x = self.scale_norm(x)
            x = self.scale_mlp(x)
    ...
    def select_action(self, x, mean_action=False):
        ...
        if scale_dist is not None:
            ...
            scale = 0.75
            # scale = 0.5
            scale_action = torch.clamp(scale_action, 1 - scale, 1 + scale)
        else:
            scale_action = torch.tensor(self.agent.env.scale_vector)
    ...
    def get_log_prob(self, states, actions):
        ...
        if scale_dist is not None:
            ...
            for ind, _ in enumerate(actions):
                if design_mask['scale_transform'][ind]:
            ...
            action_log_prob[design_mask['scale_transform']] = scale_state_log_prob
